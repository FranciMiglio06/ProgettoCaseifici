...
$required_fields = ['cli_username', 'cli_password', 'cli_nome', 'cli_cognome', 'cli_indirizzo', 'cli_email', 'cli_num_tel', 'cli_partita_iva'];

foreach ($required_fields as $field) {
    if (empty($input_data[$field])) {
        echo json_encode([
            "success" => false,
            "message" => "Campo mancante o vuoto: $field"
        ]);
        exit();
    }
}
...
$result = createCliente($cliente, $conn);
